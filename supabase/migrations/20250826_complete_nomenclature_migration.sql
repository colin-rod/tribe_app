-- Complete migration: Rename tribes to trees and circles to branches
-- This migration updates all tables, columns, foreign keys, policies, and functions

BEGIN;

-- Step 1: Drop policies that reference old table names
...
-- Step 2: Drop functions that reference old table names
...
-- Step 4: Rename columns to reflect new terminology
...
-- Step 6: Update enum types if needed (keeping existing for backward compatibility)
-- Note: We keep the existing enums as they are still valid

-- Step 7: Create updated functions with new table names
...
-- Step 8: Recreate policies with new table names
-- Branch Categories
...
-- Step 9: Create compatibility views for smooth transition
...
-- Step 10: Create compatibility function aliases
