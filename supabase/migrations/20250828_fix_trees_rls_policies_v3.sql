-- Fix missing RLS policies for trees table (v3 - drops existing policies first)
-- This addresses the 403 error when trying to create trees

-- Enable RLS on trees table if not already enabled
ALTER TABLE trees ENABLE ROW LEVEL SECURITY;

-- First, drop any existing tree policies that might conflict
DROP POLICY IF EXISTS "trees_authenticated_insert" ON trees;
DROP POLICY IF EXISTS "trees_member_select" ON trees;
DROP POLICY IF EXISTS "trees_owner_admin_update" ON trees;
DROP POLICY IF EXISTS "trees_owner_delete" ON trees;
...
DROP POLICY IF EXISTS "tree_members_own_select" ON tree_members;
